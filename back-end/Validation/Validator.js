...
function checkNoPostDataProperty(prop_name, prop_value) {
  let ret_val = false;
  switch (prop_name) {
    case datahost:
      ret_val = prop_value === "";
    ...
    case dataheaders:
      ret_val =
        ...
        Object.keys(prop_value).length === 0;
      break;
    case datamethod:
      ret_val = prop_value === "";
      break;
...
function validateDataProps({
  ...
  let datafetch_bool = datafetch === "true" ? true : false;

  let datapath_test = datafetch_bool
    ? checkProperty("datapath", datapath)
    : checkNoPostDataProperty("datapath", datapath);
  let datahost_test = datafetch_bool
    ? checkProperty("datahost", datahost)
    : checkNoPostDataProperty("datahost", datahost);
  let dataport_test = datafetch_bool
    ? checkProperty("dataport", dataport)
    : checkNoPostDataProperty("dataport", dataport);
  let datamethod_test = datafetch_bool
    ? checkProperty("datamethod", datamethod)
    : checkNoPostDataProperty("datamethod", datamethod);
  let dataheaders_test = datafetch_bool
    ? checkProperty("dataheaders", dataheaders) //bug returns 0
    : checkNoPostDataProperty("dataheaders", dataheaders);
  let databody_test = datafetch_bool
    ? checkProperty("databody", databody)
    : checkNoPostDataProperty("databody", databody); //bug
...
function validateTestSet(testset) {
  ...
  let validReqProps = false;
  let validDataProps = false;

  if (notFalsie) {
    validReqProps = validateRequestProps(testset);
    validDataProps = validateDataProps(testset);
  }
...
module.exports.validateReportObject = validateReportObject;
